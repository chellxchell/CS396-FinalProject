...
def build_wiki_relation(market_name, connection_file, tic_wiki_file,
                        sel_path_file):
    # readin tickers
    restricted_tickers = np.genfromtxt(os.path.join('..', 'data', market_name + '_tickers.csv'), dtype=str, delimiter=',', skip_header=False)
    ticker_ref = dict({})
    for ticker in restricted_tickers:
        ticker_ref[ticker] = True
    tickers = np.genfromtxt(tic_wiki_file, dtype=str, delimiter=',',
                            skip_header=False)
    ind = 0
    while ind < len(tickers):
        if tickers[ind][0] not in ticker_ref:
            tickers = np.delete(tickers, ind, 0)
        else:
            ind += 1
        ...
        for tar_item, paths in conns.items():
            for p in paths:
                ...
                if path_key in valid_path_index.keys():
                    if tar_item not in wikiid_ticind_dic or sou_item not in wikiid_ticind_dic:
                        continue
    ...
    np.save(os.path.join('..', 'data', 'relation', 'wikidata', market_name + '_wiki_relation'), wiki_relation_embedding)
