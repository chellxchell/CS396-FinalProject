    ...
    def generate_sector_relation(self, industry_ticker_file,
        ...
        for industry in valid_industry_index.keys():
            ...
            for i in range(len(cur_ind_tickers)):
                if cur_ind_tickers[i] not in ticker_index:
                    continue
                ...
                for j in range(i + 1, len(cur_ind_tickers)):
                    if cur_ind_tickers[j] not in ticker_index:
                        continue
        ...
        np.save(os.path.join(self.data_path, '..', 'relation', 'sector_industry', self.market_name + '_industry_relation'),
